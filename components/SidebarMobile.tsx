...
import { LogOut, Menu, MenuIcon, MoreHorizontal, X } from "lucide-react";
    ...
    <Sheet>
      ...
      <SheetContent hideClose className="px-3 py-4 " side="left">
        <SheetHeader className="flex flex-row justify-between items-center space-y-0">
          ...
          <SheetClose asChild>
            <Button className=" h-7 w-1 p-0" variant="ghost">
            ...
            {props.sidebarItems.links.map((link, idx) => (
              <Link key={idx} href={link.href}>
                <SideBarButton
                  variant="outline"
                  className={
                    pathname === link.href ? "border-1 border-orange-500 text-orange-500" : ""
                  }
                  icon={link.icon}
                >
                  {link.label}
